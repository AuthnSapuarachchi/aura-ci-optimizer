...
import '../App.css'; // We can share the same CSS
...
function Dashboard({ token, onLogout }) {
  ...
  const fetchAllLogs = () => {
    fetch('http://localhost:8080/api/v1/log/all', {
      method: 'GET',
      headers: { 'Authorization': `Bearer ${token}` } // Just need the token here
    })
  ...
  // Run fetchAllLogs on component mount
  useEffect(() => {
    fetchAllLogs();
  }, []);
  ...
  const handleSubmit = () => {
    ...
    fetch('http://localhost:8080/api/v1/log/upload', {
      method: 'POST',
      headers: authHeaders, // <-- USE OUR AUTH HEADERS
      body: logText,
    })
      .then(response => response.json())
      .then(data => {
